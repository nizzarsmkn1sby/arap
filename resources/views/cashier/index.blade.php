...
@section('title', 'Terminal Kasir | ARAP POS')
...
<div class="grid cashier-layout">
    ...
    <div>
        <div class="card" style="padding: 20px; margin-bottom: 30px; background: rgba(31, 41, 55, 0.2);">
            <form action="{{ route('pos.index') }}" method="GET" style="display: flex; gap: 15px;">
                <div style="flex: 1; position: relative;">
                    <span style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: var(--text-muted);">🔍</span>
                    <input type="text" name="search" class="form-control" placeholder="Cari produk berdasarkan nama atau SKU..." value="{{ request('search') }}" 
                ...
                @if(request('search'))
                    <a href="{{ route('pos.index') }}" class="btn" style="height: 50px; background: rgba(239, 68, 68, 0.1); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.2);">Reset</a>
                @endif
            </form>
        </div>
        ...
        <div class="grid grid-3" style="gap: 20px; max-height: 65vh; overflow-y: auto; padding-right: 10px;">
            ...
            <div class="card" style="padding: 20px; display: flex; flex-direction: column; align-items: center; text-align: center; cursor: pointer; border: 1px solid var(--border);" 
                 onclick="tambahKeKeranjang({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }})">
                ...
                <h4 style="font-size: 15px; font-weight: 700; color: #fff; margin-bottom: 8px; line-height: 1.4;">{{ $product->name }}</h4>
                <div style="font-size: 16px; color: var(--primary); font-weight: 800; margin-bottom: 10px;">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                <div style="font-size: 12px; color: var(--text-muted); font-weight: 600; background: rgba(255,255,255,0.03); padding: 4px 12px; border-radius: 8px;">Stok: {{ $product->stock_quantity }}</div>
            </div>
            @empty
